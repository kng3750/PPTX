...
def _copy_slide_background(template_slide, new_slide):
    """템플릿 슬라이드의 배경을 새 슬라이드에 복사"""
    try:
        ns = "{http://schemas.openxmlformats.org/presentationml/2006/main}"
        t_csld = template_slide.part.element.find(f"{ns}cSld")
        n_csld = new_slide.part.element.find(f"{ns}cSld")
        if t_csld is None or n_csld is None:
            return
        t_bg = t_csld.find(f"{ns}bg")
        if t_bg is not None:
            n_bg = n_csld.find(f"{ns}bg")
            if n_bg is not None:
                n_csld.remove(n_bg)
            n_csld.insert(0, copy.deepcopy(t_bg))
    except Exception:
        pass
    ...
    for _, value in template.part.rels.items():
        if "notesSlide" not in value.reltype:
            try:
                ...
                pass
    _copy_slide_background(template, new_slide)
    return new_slide
